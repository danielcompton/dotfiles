# From https://github.com/anishathalye/dotfiles/blob/c98622d4da7ad3147fefedfb3a43149d8ed3cac7/zsh/prompt.zsh
# User customizable options
# PR_ARROW_CHAR="[some character]"
RPR_SHOW_USER=false # (true, false) - show username in rhs prompt
RPR_SHOW_HOST=false #(true, false) - show host in rhs prompt
...
function PCMD() {
    if [[ "${PROMPT_MODE}" == 0 ]]; then
        echo "$(PR_DIR) $(PR_ERROR)
$(PR_ARROW) " # space at the end
    elif [[ "${PROMPT_MODE}" == 1 ]]; then
...
RPROMPT='$(RCMD)' # set asynchronously and dynamically
...
function precmd() {
    function async() {
        # save to temp file
        printf "%s" "$(RCMD)" > "/tmp/zsh_tmp_prompt_${PPID}"
...
function TRAPUSR1() {
    # read from temp file
    RPROMPT="$(cat /tmp/zsh_tmp_prompt_${PPID})"
